...
<body>
	<div class="place">
		<span>位置：</span>
		<ul class="placeul">
			<li><a href="javascript:void(0)">浏览历史管理</a></li>
		</ul>
	...
	<div class="rightinfo">
		...
		<tbody>
			<c:forEach items="${paperList}" var="paperList">
				<tr>
					<td>${paperList.id}</td>
					<td>${paperList.browse_id}</td>
					<td>${paperList.browse_time}</td>
					<td>${paperList.browse_shop }</td>
					<td>${paperList.sid }</td>
					<td>
						<a href="javascript:void(0)" id="delete" onclick="delete_order(${paperList.id})">删除</a>
					</td>	
				</tr>
			</c:forEach>
	</table>
...
<!--  <span style="color:red;font-size:20px;" id="deleteSuccess">发</span> -->
		...
		/* 查询的点击事件 */
			$("#selectActive").click(function(){
				...
				window.location.href="<%=basePath%>/zcp/selectActive.shtml?title1="+sTitle+"&type1="+sType+"";
			});
			/*改变每页显示条数的事件 */
			$("#sPageSize").change(function(){
				...
				var pageSize1=$("select[name=pageSize]").val().trim();
				 window.location.href="<%=basePath%>/zcp/selectActive.shtml?title1="+sTitle1+"&type1="+sType1+"&pageSize="+pageSize1+"";
			});
            /* 获取存储的当前页数 */
			...
			var pageNoClick=$(this).children().html();
				if(pageNo!=pageNoClick){
					...
					var sType1=$("select[name=type1]").val().trim();
					var pageSize1=$("select[name=pageSize]").val().trim();
					window.location.href="<%=basePath%>/zcp/selectActive.shtml?title1="+sTitle1+"&type1="+sType1+"&pageSize="+pageSize1+"&pageNo="+pageNoClick+"";
				}
			});
		</script>
		<div class="tip">
